...
%according to the spec modification (Clip, flip, square, shift) and data 
%orientation 'raw - Unchanged', 'normalised', and 'normalised+centralised')
%The files containing the results for each spectral modication made to the
%kernels are stored as RES_clip, RES_flip, RES_Square, RES_Shift and RES_Raw
%==========================================================================

%specify location of files and load the valid kernels list
load('./Pep_K7/NsdspecFileName')
file = './Pep_K7/k_ed3_1_1.mat';

[fpath, fname, ext] = fileparts(file); 
%==========================================================================
%Step necessary to get the list of files containing the clasification
%result
% for i=1:size(validIndeKernels,1)
%     SpecName{i,1} = sprintf('Result_%s',validIndeKernels{i});
% end

%==========================================================================
%Load a sample file to analyse
%Extract the values for C
...
%==========================================================================
%iterate through the result files and extract the results for the kernel
%manipulation (untouched, centering and normalization) and the spectral
%modifications (none, clip, flip, shift, square)
...
for k=1:length(specMode)
    ...
    %save the files as RES_clip, RES_flip, etc
    Xname = sprintf('RES_%s',specMode{k});
    breakDown = fullfile(fpath,Xname);
    save(breakDown,'A')
    Xname=[]; A={};
    
    %Qname = fullfile(fpath,'SpecName');
    %save(Qname,'SpecName')
