function [B, A,mse, auc] = plotroc(y,x,params)
% auc1 = plotroc(heart_scale_label, heart_scale_inst, model)
% 
% y= heart_scale_label
% x= heart_scale_inst
% params = model
...
% 	plotroc(y,x,model);
	...
	elseif exist('params') && ~ischar(params)
        ...
        %[predict_label,mse,deci] = svmpredict(y,x,model); % the procedure for predicting
        %[predict_label,mse,deci] = svmpredict(y,x,model); % the procedure for predicting
        [predict_label,mse,deci] = svmpredict(y, [(1:n1)',x],model); 
		[A, auc] = roc_curve(deci*model.Label(1),y);
        [~, B] = EvaluTest2(y, predict_label)
	else
		if ~exist('params')
			params = [];
		end
...
function [A , auc] = roc_curve(deci,label_y)
