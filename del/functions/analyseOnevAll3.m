...
function [PerfEval, PerfEvalFull] = analyseOnevAll3(G,s,L)
...
%@Nanomsky
   ...
   % This script follows on from the 'mPredict' script. It extracts the
   % performance according to the  spectral modifcation applied

   %Input
   %------
   % G = Cell from 'mPredict' script. Contains the predictions made for
   %     each of the spectral modifications made
   % s = index for number of primary kernels in G. Used when the experiment
   %     has different kernel sizes
   % L = Used to indicate is the kernel was PSD and therefore does not
   %     contain any predictions from spectral modification
   %
   %Output
   %-------
   % PerfEval - Contains the outcome of the EvaluTest2 performance
   % evaluation for te various forms of modifications to make the kernel
   % PSD
   % PerfEvalFull - Contains a full version of PerfEval
