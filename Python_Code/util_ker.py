...
def splitdata2(X, Y, rand_seed, tnx):
    '''
    Function used to split data into training, test and validation datastes
    This takes the predictor variables X and response variables Y, and 
    ...
    train_index = index[0:len1]
    valid_index = index[len1:(len1 + len2)]
    test_index = index[(len1 + len2):]
    ...
    for i in train_index:
        xtr.append(Examples[i])
    
    for k in valid_index:
        xva.append(Examples[k])
    
    for j in test_index:
        xte.append(Examples[j])
    ...
    print(f'{len(xva)} validation examples')
    print(f'{len(xte)} testing examples')
...
def addIndexToKernel(K_mat):
    ...
    A requirement for using LiBSVM
    '''
    n = K_mat.shape[0]
...
def fitkernelLOOCV(xtr, ytr, kernel, params):
    ...
    for ker in kernel:
        ...
        for par in range(len(params[ker])):
            ...
            scores       = cross_val_score(svm_model, K_mat, ytr, scoring='accuracy', cv=cv, n_jobs=-1)
            Stage2       = (np.mean(scores), np.std(scores), scores)
            print('--> Bag-of-Words Accuracy: %.3f (%.3f)' % (np.mean(scores), np.std(scores)))
            
            #################################################################################
            #Predict
            Y_pred   = cross_val_predict(svm_model, K_mat, ytr, cv=cv, n_jobs=-1) 
            Perf_eva = EvaluateTest(Y_pred, np.squeeze(ytr))#Extract evaluation metric on Train set
        
            Stage3   = (Y_pred, Perf_eva)
            #################################################################################
            
            Result[str(ker) +'_' + str(par)] = (Stage1, Stage2, Stage3)
...
def Analyse_ResultsLOOCV(Exp):
    ...
    #Extract the remaining variables and append to the relevant list
    for i in range(n):
        Eva.append(list(Stage3[i][1].values()))
        train_kernel_time.append(Stage1[i][1])
        #test_kernel_time.append(Stage2[i][1])
        diag_Dominance.append(Stage1[i][2])
        is_PSD.append(Stage1[i][3])
    ...
    #Extract the scores
    for i in range(n):
        mean_scores.append(Stage2[i][0])
        std_scores.append(Stage2[i][1])
        scores.append(Stage2[i][2])
    score_df = pd.DataFrame(zip(mean_scores,std_scores))
    score_df.columns = ['mean score', 'std']
    score_df.index   = col_names
    
    #Store the extracted data in DataFrames
    perfEva = pd.DataFrame(Eva)
    perfEva.columns = [key for key in Stage3[0][1].keys()]
    perfEva.index = col_names 
    
    #print(perfEva)
    Kernel_Analysis = [train_kernel_time, diag_Dominance, is_PSD]
    
    Kernel_index    = ['train_kernel_time','diag_Dominance','is_PSD']
    ...
    #print(Kernel_Analysis_df)
    return perfEva, Kernel_Analysis_df, score_df,scores
